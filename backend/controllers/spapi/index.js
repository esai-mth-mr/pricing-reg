...
// const { getCatatalogItem, getUploadListingItems } = require("./uploadListings");
const { getUploadListingItems } = require("./uploadListings");
...
const patchBusinessPrice = async (sku, marketplaceId, currency, price) => {
  try {
    let resData = await sellingPartner.callAPI({
      operation: "patchListingsItem",
      endpoint: "listingsItems",
...
const setMockAmazonData = async () => {
  marketPlaces.map(async (marketplaceName) => {
    ...
    const updateState = await putAmazonDatabase(
      marketplaceName,
      content,
      businessPrice
    );
    if (updateState) {
      ...
      console.log(`${marketplaceName} have no content to update by any error.`);
    }
  });
};
